...
export default function Layout({ children, Progress, isText, contentMaxWidth }) {
  return (
    <>
      <Head>
      ...
      >
        <Header 
          ...
          Progress={Progress} 
        />
        <Content contentMaxWidth={contentMaxWidth}>
          { children }
        </Content>
      </Flex>
    </>
...
export function Content({ children, contentMaxWidth }) {
  return (
    <Flex alignItems="start" minHeight={[`calc(100vh - ${titleBarHeightMob})`, `calc(100vh - ${titleBarHeightDesk})`] } pos="relative" py={5}>
      <Container centerContent maxW={contentMaxWidth || "container.sm"} px={5} py={["65px", "97px"]}>
        {children}
      </Container>
    </Flex>
  )
